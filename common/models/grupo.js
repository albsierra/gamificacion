...
// TODO permitir la recepción de un String con los emails separados por comas.
    this.juego((err, juego) => {
      ...
      if (!juego.grupal) callback(new Error(
        'El juego no permite invitaciones a grupos'
      ));

      gamificaFunctions.filtraInvitados(
        emails, req, Grupo.app, (err, emails) => {
          if (err) callback(err);
    ...
    return new Promise((resolve, reject) => {
      ...
      grupo.invitaciones.create({
        ...
        var url = 'http://' + config.host + ':' +
          config.port + '/' + config.restApiRoot + '/Grupos/' + grupo.id +
          '/aceptarInvitacion?token=' + token;
        var html = 'Click <a href="' + url + '">aqu&iacute;</a> ' +
          'para aceptar la invitación al grupo ' + grupo.nombre;
  ...
  Grupo.prototype.aceptarInvitacion = function (token, req, callback) {
    ...
    grupo.invitaciones.findOne({
      ...
      if (!invitacion) callback(new Error(
        'El token enviado no corresponde a ninguna invitación para este grupo')
      );
  ...
  Grupo.afterRemote('prototype.aceptarInvitacion',
    function (context, miembro, next) {
      ...
      grupo.invitaciones.findOne({
        ...
        if (!invitacion) next(new Error(
          'El token enviado no corresponde a ' +
          'ninguna invitación para este grupo'
        ));
