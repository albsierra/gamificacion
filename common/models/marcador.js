...
module.exports = function (Marcador) {
  Marcador.validateAsync('puntos', puntosMenorIgualMaximo, {
    message: 'Los puntos superan el máximo para esta prueba',
  });
  ...
  Marcador.prototype.juegoAlQuePertenece = function (cb) {
    this.prueba(function (err, prueba) {
      if (err) cb(err);
      if (!prueba) {
        cb(new Error('No existe la prueba asociada a este Marcador'));
      }
